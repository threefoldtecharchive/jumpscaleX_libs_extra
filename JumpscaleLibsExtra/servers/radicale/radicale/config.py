...
INITIAL_CONFIG = OrderedDict(
    [
        (
            ...
            OrderedDict(
                [
                    ...
                    (
                        "max_connections",
                        {"value": "20", "help": "maximum number of parallel connections", "type": positive_int},
                    ),
                    (
                        "max_content_length",
        ...
        (
            ...
            OrderedDict(
                [
                    ...
                    ("htpasswd_filename", {"value": "/etc/radicale/users", "help": "htpasswd filename", "type": str}),
        ...
        (
            ...
            OrderedDict(
                [
                    ...
                    (
                        "file",
                        {"value": "/etc/radicale/rights", "help": "file for rights management from_file", "type": str},
                    ),
                ]
            ),
        ),
            ...
            OrderedDict(
                [
                    ...
                    (
                        "filesystem_folder",
                        {
                            "value": os.path.expanduser("/var/lib/radicale/collections"),
                    ...
                    (
                        "max_sync_token_age",
                        {"value": 2592000, "help": "delete sync token that are older", "type": int},  # 30 days
                    ),
                    (
                        "filesystem_fsync",
                    ...
                    (
                        "filesystem_locking",
                        {"value": "True", "help": "lock the storage while accessing it", "type": bool},
                    ),
                    (
        ...
        (
            ...
            OrderedDict(
                [
                    ("config", {"value": "", "help": "logging configuration file", "type": str}),
                    (
                        "debug",
                        {
                            "value": "False",
                    ...
                    ("full_environment", {"value": "False", "help": "store all environment variables", "type": bool}),
...
def load(paths=(), extra_config=None, ignore_missing_paths=True):
    ...
    for path in paths:
        if path or not ignore_missing_paths:
            try:
                ...
                raise RuntimeError("Failed to load config file %r: %s" % (path, e)) from e
    ...
    for section in config.sections():
        ...
        allow_extra_options = "type" in INITIAL_CONFIG[section] and config.get(section, "type") not in INITIAL_CONFIG[
            section
        ]["type"].get("internal", ())
        for option in config[section]:
            if option not in INITIAL_CONFIG[section]:
                if allow_extra_options:
                    continue
                raise RuntimeError("Invalid option %r in section %r in " "config" % (option, section))
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "%r" % (type_.__name__, option, section, config.get(section, option))
                ) from e
    return config
