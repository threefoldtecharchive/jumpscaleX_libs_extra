...
def start(name="radicale", filename=None, debug=False):
    ...
    if filename:
        ...
        try:
            ...
            raise RuntimeError("Failed to load logging configuration file %r: " "%s" % (filename, e)) from e
        ...
        if hasattr(signal, "SIGHUP"):
            ...
            def handler(signum, frame):
                try:
                    ...
                    logger.error("Failed to reload logging configuration file " "%r: %s", filename, e, exc_info=True)

            signal.signal(signal.SIGHUP, handler)
    else:
        # Default configuration, standard output
        handler = logging.StreamHandler(sys.stderr)
        handler.setFormatter(logging.Formatter("[%(thread)x] %(levelname)s: %(message)s"))
